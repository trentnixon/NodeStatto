...
export default class ForandAgainstTable extends Component {
    ...
    componentWillMount() {}
      ...
      find(data, variable, Startvalue,eq){
            ...
            data.map((team,int)=>{
                if(eq === "<"){
                    console.log(team[variable], value );

                    if(team[variable] < value && isFinite(team[variable])  && team[variable] !== 0){
                        pointer=int;
                        value = team[variable]
                    }
                }
                                ...
                                this.state.stats.map((game,i)=>{
                                    return(
                                            <div className="tr" key={i}>
                                                ...
                                                <div className="td">
                                                    { this.find(this.props.Data.FORAGAINST.For,game.Name[0],game.sv,game.eq ) }
                                                </div>
                                                <div className="td">
                                                    { this.find(this.props.Data.FORAGAINST.Against,game.Name[0],game.sv,game.eq ) }
                                                </div>
                                            </div> 
                                    )
                                })
