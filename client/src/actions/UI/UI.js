...
export function FilterDataSeries(DATA,Filter){
    ...
    if(Filter.Year === 'Career' && parseFloat(Filter.LW) === 0){ 
            return DATA; 
        }
    else if(Filter.Year === 'Career' && parseFloat(Filter.LW) !== 0 )
        { 
            return DATA.filter(function(v, i) { return (v["Meta"]["LW"] === Filter.LW ); });
        }
    else if(Filter.Year !== 'Career' && parseFloat(Filter.LW) === 0)
        { 
            return DATA.filter(function(v, i) { return (v["Meta"]["Year"] === Filter.Year ); });
        }
...
/**
 * 
 *  This function Creates the Data Label for Graphs for the function above
 *  if Needle === Career then the full data set is sent back
 *  
 */
...
export const CreateGraphDates = (Data,Path)=>{
            let Series=[];
            Data.map((game,i)=>{
